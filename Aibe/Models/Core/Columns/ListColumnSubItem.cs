...
namespace Aibe.Models.Core {
  public class ListColumnSubItem : BaseInfo {
    ...
    public int Width { get; private set; } = ListColumnInfo.DefaultWidth; //assuming default width unless specified otherwise
    private TableValueRefInfo refInfo { get; set; }
    ...
    public ListColumnSubItem(string desc, char subItemType, int width) : base(desc) { 
    ...
    private bool isValidReference(string desc) {
      ...
      if (parts == null || parts.Count < 3 && parts.Count > 5) //can only consists of 3 to 5 parts. Support additional where clause
        return false;
      if (!parts[0].EqualsIgnoreCase(DH.Ref))
        return false;
      int index = desc.IndexOf(':');
      string refString = desc.Substring(index + 1).Trim();
      TableValueRefInfo refInfo = new TableValueRefInfo(refString);
